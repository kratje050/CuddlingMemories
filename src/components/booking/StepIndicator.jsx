...
export default function StepIndicator({ current, onStepSelect }) {
  return (
      ...
      {labels.map((label, index) => {
        ...
        return (
          <li key={label} className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => isDone && onStepSelect?.(index)}
              disabled={!isDone || !onStepSelect}
              title={isDone ? `${label} aanpassen` : undefined}
              aria-label={isDone ? `${label} aanpassen` : label}
              ...
              } ${isDone && onStepSelect ? "cursor-pointer hover:border-cocoa hover:bg-cocoa hover:text-card" : "cursor-default"}`}
            >
              {index + 1}
            </button>
            {isDone && onStepSelect ? (
              <button type="button" onClick={() => onStepSelect(index)} className="transition hover:text-cocoa hover:underline">
                {label}
              </button>
            ) : (
